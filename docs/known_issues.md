...
## ✅ Resolvidos em 2026-08-10

| # | Problema | Resolução |
|---|---|---|
| S3 | **Fluxo antigo de upload específico da Cora (`CORA_CLIENT_ID` fixo, janela Jan-Jun/2026, aba "Overrides Históricos" do hub) não foi generalizado junto com o redesenho de `client_reporting_source_config`.** | Removido em `hub/app.py` (commit `a571516`) — o fluxo hardcoded só-Cora foi apagado, fica só o fluxo genérico (upload → comparação → config por cliente, ver comentário no topo da Aba 8 em `hub/app.py`). Não há mais dois caminhos paralelos para o mesmo tipo de dado. |
| G9 | **`core.resolve_client_business_rule()` não podia ser chamada diretamente de `gold.fact_pacing`** — `fact_pacing.sql` reimplementava a função inline (JOIN real + `ARRAY_AGG`, CTEs `rule_candidates`/`rule_resolved`) em vez de chamar `core.resolve_client_business_rule()` como as outras 3 tabelas de regra fazem com suas `resolve_*` (desvio do ADR-0001). Causa raiz passou por 3 hipóteses: (1ª, descartada) `UNION ALL`/`FULL OUTER JOIN` a montante — descartada materializando `stg.fact_pacing_base` como tabela física (commit `f50ec8a`), erro reproduziu igual; (2ª, descartada) `ORDER BY CASE WHEN client_id = p_client_id...` referenciando o parâmetro correlacionado dentro do `ARRAY_AGG` — descartada por teste isolado, o erro persiste mesmo sem `CASE` nenhum; (3ª, confirmada) `ARRAY_AGG(...ORDER BY...LIMIT...)` nunca decorrelaciona numa SQL UDF chamada de forma correlacionada em BigQuery, independente do que o `ORDER BY` referencia — só agregação simples (`MIN`/`MAX`/`ANY_VALUE`, sem `ORDER BY`/`LIMIT`) decorrelaciona, e só 1 por chamada. | `core/ddl/resolve_client_business_rule.sql` reescrita (commit `88c09d6`) para usar um único `MAX()` sobre uma STRING codificada (especificidade + data AAAAMMDD + JSON serializado, prefixo de largura fixa) — `MAX()` escolhe a linha certa por ordenação lexicográfica, `SUBSTR`+`PARSE_JSON` reconstitui o payload. `gold/ddl/fact_pacing.sql` chama a função diretamente, workaround `rule_candidates`/`rule_resolved` removido. Testado em `douglas-bq-staging`: chamada isolada + 8 cenários de vigência/prioridade/pausa/fronteira de data + caso real (`banco_cora_fe13d78a`/Native/2026-01-12) — todos batendo. Nada aplicado em produção ainda. |
