...
## ✅ Resolvidos em 2026-08-09

| # | Problema | Resolução |
|---|---|---|
| A4 | **`fct_newad_bet_daily.sql`, `fct_luckbet_delivery_daily.sql`, `fct_delivery_daily_mvp.sql` → `share.io_calc_daily_v4` (Shiro), fora do `OWNERSHIP.yaml`.** | Reconfirmado ao vivo em 2026-08-09 (bloqueado em 2026-08-08 por falta de acesso): nenhuma das 3 views (`fct_newad_bet_daily`, `fct_luckbet_delivery_daily`, `fct_delivery_daily_mvp`) existe em `adframework.gold` hoje (`INFORMATION_SCHEMA.TABLES` — zero linhas). `share.io_calc_daily_v4` também não existe (dataset `share` existe, mas a tabela não). Os 3 arquivos `.sql` continuam no repo (`gold/delivery/`) mas não correspondem a nenhum objeto vivo — o risco descrito (consumidor exposto a mudança silenciosa do Shiro) é hoje inexistente na prática, porque nada lê essas views. Não confundir com "resolvido pela correção" — é "resolvido porque o objeto nunca chegou a ficar ativo/foi descontinuado sem deixar rastro de quando". |
| A5 | **`gold.fct_creative_daily` — objeto fantasma, sem DDL em lugar nenhum.** | Reconfirmado ao vivo em 2026-08-09: `gold.fct_creative_daily` não existe em `adframework.gold` (`INFORMATION_SCHEMA.TABLES` — zero linhas). `gold/creative/fct_luckbet_creative_daily.sql` (que lê dela) também não está deployado. Classificação: código morto nunca promovido a produção — não é objeto do Shiro (não apareceu em nenhuma verificação do Admin UI), é um workaround do próprio repo que nunca chegou a rodar. |
| C1 | **`docs/_legacy/id_dependency_map.md` marcado como "dropado, não usar" — mas descrevia cadeias que 6 views gold ativas hoje ainda consomem.** | Reconfirmado ao vivo em 2026-08-09 (bloqueado em 2026-08-08). Das 5 tabelas/views citadas: `stg.io_lines_v4`, `share.io_calc_daily_v4`, `share.platform_daily_detail` e `share.newad_operational_daily` **não existem** (confirmado, `INFORMATION_SCHEMA.TABLES` dos datasets `stg`/`share` — zero linhas cada). `core.io_binding_registry_v4` **ainda existe** como VIEW — única sobrevivente das 5. Das 6 views gold citadas como consumidoras (`fct_cora_delivery_full`, `fct_luckbet_delivery_full`, `fct_newad_fintech_daily`, `fct_newad_bet_daily`, `fct_luckbet_delivery_daily`, `fct_delivery_daily_mvp`), **nenhuma existe em `adframework.gold`** hoje (`INFORMATION_SCHEMA.TABLES` — zero linhas para as 6). Conclusão: o aviso do doc estava certo na prática — as 6 views "ativas" citadas não estão de fato ativas em produção, e não há exposição de risco hoje. `core.io_binding_registry_v4` sobrevivendo sozinha não muda a conclusão, porque nenhuma view oficial da gold a referencia (já confirmado em A2). Nenhum arquivo `docs/_legacy/id_dependency_map.md` foi encontrado no repo nesta sessão (pasta `docs/_legacy/` não o contém mais) — pode já ter sido removido/consolidado em sessão anterior sem atualizar esta entrada; não impacta a conclusão acima, que foi tirada direto do `INFORMATION_SCHEMA` ao vivo, não do conteúdo do doc legado. |
...
| S5 | 🟡 **`scripts/deploy/apply_ddl.py` — `CREATE_STATEMENT_RE` (linha ~165-173) não reconhece `CREATE TABLE IF NOT EXISTS`, só casa `CREATE [OR REPLACE] TABLE`.** Achado em 2026-08-09 ao aplicar `core/ddl/client_business_rules.sql` (usa `IF NOT EXISTS`) em `douglas-bq-staging --env=test`: não bloqueou desta vez por coincidência (fallback por caminho de arquivo + o `--project` alternativo cobriram o caso), mas em fluxo normal (`--project` default = produção, `--env=test` com sufixo `_test`) o regex não bater teria feito `creation_object` ser derivado do nome do arquivo em vez da instrução `CREATE` real. | Severidade baixa/moderada — não bloqueou o teste real, mas pode gerar `creation_object` errado em `core.schema_change_log` (rastreabilidade de qual objeto foi de fato criado) em outro cenário com `IF NOT EXISTS` e sem os mesmos fallbacks presentes. | Estender `CREATE_STATEMENT_RE` para casar `CREATE TABLE IF NOT EXISTS` (e o equivalente para função/view, se aplicável). Não corrigido ainda — só documentado. |
| S6 | 🟢 **`bq add-iam-policy-binding` falhou com "This feature requires allowlisting" durante o deploy manual de emergência do hub em 2026-08-10.** Não bloqueou o deploy porque a service account (`douglas-data-hub-sa`) já tinha o binding de execução de uma configuração anterior. | Severidade baixa — hoje sem impacto. Mas se um binding IAM novo for necessário no futuro (nova SA, novo dataset, nova permissão) via esse comando/conta/projeto, pode falhar do mesmo jeito e bloquear de verdade. | Se `add-iam-policy-binding` for necessário de novo e falhar assim, usar o console GCP (IAM > Adicionar principal) como alternativa, ou solicitar allowlisting da feature para esta conta/projeto. |
...
| G9 | **`core.resolve_client_business_rule()` não pode ser chamada diretamente de `gold.fact_pacing` — causa raiz real confirmada 2026-08-10, diferente da hipótese anterior.** `gold/ddl/fact_pacing.sql` reimplementa a função inline (JOIN real + `ARRAY_AGG`, CTEs `rule_candidates`/`rule_resolved`) em vez de chamar `core.resolve_client_business_rule()` como as outras 3 tabelas de regra fazem com suas `resolve_*` (ver ADR-0001). Hipótese original (cadeia de `UNION ALL`/`FULL OUTER JOIN` a montante causando "Correlated subqueries...") foi testada e descartada na task "Materializar base de fact_pacing na STG" (commit `f50ec8a`): materializar o cruzamento planejado×realizado em `stg.fact_pacing_base` (tabela física, sem JOIN/UNION nenhum) não resolveu — o erro reproduz idêntico numa query mínima contra a tabela física pura. Causa raiz real, isolada com 2 `TEMP FUNCTION`s de teste: dentro de `core/ddl/resolve_client_business_rule.sql`, o `ARRAY_AGG(... ORDER BY CASE WHEN client_id = p_client_id THEN 0 ELSE 1 END ...)` referencia o **parâmetro correlacionado da função dentro do próprio `ORDER BY`** — é isso, não o `UNION`/`JOIN` a montante, que o BigQuery recusa decorrelacionar. `resolve_dict_format`/`resolve_platform_rule` (as outras 2 `resolve_*`, que funcionam direto em `fact_io_plan.sql`) não têm esse padrão — usam `MIN()`/`ANY_VALUE()` simples, sem referenciar parâmetro correlacionado dentro do `ORDER BY` de uma agregação. | O workaround inline em `fact_pacing.sql` funciona e está em produção-teste (staging), mas qualquer view GOLD futura que precise de `client_business_rules` vai precisar do mesmo workaround manual em vez de reusar a função — perde a centralização que motivou o ADR-0001 em primeiro lugar. | Reescrever `core.resolve_client_business_rule()` para não referenciar o parâmetro correlacionado dentro do `ORDER BY` (ex: 2 chamadas separadas — uma só para o override do cliente, outra só para a regra geral — com `COALESCE` entre elas, cada uma com agregação simples). Fora de escopo da task que descobriu isso — decisão de priorização pendente do Douglas. |
