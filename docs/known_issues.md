...
| S4 | ⚠️ **Nota 2026-08-06 manhã — a landing RAW literal deixou de ser "exceção" (ver banner no topo do arquivo): com a decisão final, `douglas-bq-staging.raw` é sempre físico, então este landing passa a ser o padrão, não um caso especial.** A "ferramenta de carga de dado histórico" foi desenhada como 2 peças e só 1 foi construída. Desenho fechado na madrugada de 2026-08-05→06 (nunca escrito no Notion até agora — só existia na conversa): planilha crua do comercial → sobe no Hub **sem nenhum tratamento** → landing como **RAW literal dentro do `douglas-bq-staging`** (não produção — é trabalho de descoberta/construção, mora em staging por desenho) → Douglas + IA analisam juntos a estrutura real (nomes de coluna, amostra, o que cada coisa significa — não automatizável de antemão, cada cliente chega diferente) → constrói-se um **mapeamento de normalização versionado, específico daquele cliente** → só então promove pra `core.historical_overrides_delivery`, com a RAW crua permanecendo intacta como prova. Ou seja, a ferramenta = (a) subir/visualizar arquivo cru no Hub sem transformar nada + (b) mecanismo de normalização construído cliente a cliente. **O que o backend construiu (`scripts/deploy/load_historical_override.py`) é só a metade (b)**: o script exige um arquivo **já normalizado** como entrada (CSV/parquet com as colunas exatas do schema final) — toda a análise/normalização acontece fora do BigQuery, sem nenhum landing RAW em lugar nenhum. Confirmado lendo o código-fonte do script, não suposição. | Sem a peça (a), não existe RAW literal pra auditar/reprocessar se a normalização errar, e não existe visualização no Hub da estrutura real do arquivo — a análise "conjunta" vira um passo manual fora de qualquer ferramenta, sem rastro no BigQuery. | Construir a peça (a): upload no Hub → landing literal em `douglas-bq-staging` (dataset `raw`, tabela por cliente/arquivo, sem parsing) → tela mostrando estrutura real (colunas, amostra, contagem) pra apoiar a análise conjunta antes de escrever o mapeamento de normalização. |
