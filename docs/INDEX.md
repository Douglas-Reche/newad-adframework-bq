...
| `core_layer_design.md` | ✅ ATUAL | 2026-08-09 | Design da CORE layer (lacuna preenchida — RAW/STG/GOLD já tinham doc próprio, CORE não): narrativa (versionamento SCD2, funções `resolve_*`, divergência staging×produção do override histórico) + inventário verificado ao vivo dos 13 objetos `owner: pipeline` (tabelas + views). Não duplica a lista de objetos `owner: shiro_admin_ui`, que continua só em `core/OWNERSHIP.yaml` |
