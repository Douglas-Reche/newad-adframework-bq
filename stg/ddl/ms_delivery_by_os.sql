-- stg.ms_delivery_by_os (T11)
-- Entrega MediaSmart segmentada por sistema operacional.
-- Grain: day + ms_client_id + ms_campaign_id + ms_strategy_id + operating_system + conversion_source
-- Fonte: raw.mediasmart_delivery_by_os (operacional desde 2026-06-11, backfill jan–jun/2026)
--   273.799 linhas | 2026-01-01 → hoje | cron 03:50 UTC
--
-- Mapeamento: drilldown param 'os' → API header "Operating system" → operating_system
-- Valores típicos: android, ios, windows, other
--
...
CREATE OR REPLACE VIEW `adframework.stg.ms_delivery_by_os` AS
SELECT
...
FROM `adframework.raw.mediasmart_delivery_by_os` d
