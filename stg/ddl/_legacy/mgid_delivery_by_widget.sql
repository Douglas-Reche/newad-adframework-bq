-- stg.mgid_delivery_by_widget (T13 MGID)
-- Entrega MGID segmentada por widget (publisher placement).
-- Grain: day + mgid_campaign_id + widget_id
-- Fonte: raw.mgid_stats_by_widget (Job G) — 2025-10-01 → hoje
-- Dedup: ROW_NUMBER por raw_ingested_at DESC (backfill rodou 2x)
-- Depende de: core.platform_client_links
--
-- widget_id: ID numérico do publisher placement (widgetid na API MGID)
-- Equivalente conceitual ao publisher da MediaSmart — porém não há JOIN possível:
--   MS identifica por company+url+exchange (texto), MGID por ID numérico.
-- Sem conversões: não disponível neste grain na API MGID (mesmo padrão da MS publisher).
