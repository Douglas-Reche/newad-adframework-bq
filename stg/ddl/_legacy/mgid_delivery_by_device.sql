-- stg.mgid_delivery_by_device (T9 MGID)
-- Entrega MGID segmentada por tipo de dispositivo.
-- Grain: day + mgid_campaign_id + device_type
-- Fonte: raw.mgid_stats_by_device (Job C) — 2025-10-01 → hoje
-- Dedup: ROW_NUMBER por raw_ingested_at DESC (backfill rodou 2x)
-- Depende de: core.platform_client_links
--
-- Device types confirmados: mobile, desktop, tablet, smarttv
-- Sem app_vs_web, strategy_id, conversion_source (exclusivos MediaSmart).
-- Sem video quartis (API MGID não retorna por device).
-- Financeiro incluído neste grain (necessário para análise gasto por device).
-- spent/revenue/profit: Python dict → parser REPLACE+JSON_VALUE. roas: inteiro direto.

CREATE OR REPLACE VIEW `adframework.stg.mgid_delivery_by_device` AS

WITH stats_deduped AS (
  SELECT *,
    ROW_NUMBER() OVER (PARTITION BY day, campaignid, devicetype ORDER BY raw_ingested_at DESC) AS rn
  FROM `adframework.raw.mgid_stats_by_device`
...
SELECT
  ...
  'stats_by_device'                                                AS source_table
FROM parsed p
