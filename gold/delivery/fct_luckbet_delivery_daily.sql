CREATE OR REPLACE VIEW `adframework.gold.fct_luckbet_delivery_daily` AS
...
SELECT
  ...
  -- Plataforma e campanha
  d.platform,
  d.platform_campaign_id,
  d.platform_campaign_name AS nome_campanha_plataforma,
  d.platform_strategy_id,
  d.platform_strategy_name AS estrategia_plataforma,
  -- Contexto do IO (espelho da planilha Lille)
  d.line_item AS estrategia,
  d.creative_type AS formato,
  d.target_device AS device,
  d.buying_model AS modelo_compra,
  CAST(d.buying_value AS NUMERIC) AS valor_unitario_previsto,
  d.deliverable_metric AS tipo_entrega,
  ...
  CAST(d.planned_volume_daily AS NUMERIC) AS volume_diario_previsto,
  CAST(d.planned_volume_to_date AS NUMERIC) AS volume_acumulado_previsto,
  d.active_days AS dias_ativos,
  -- Entrega
  d.impressions AS impressoes,
  d.clicks,
  d.video_start AS video_inicio,
  d.video_completed AS video_conclusao,
  -- Conversões Luckbet (confirmado Shiro 29/04/2026)
  ...
  d.line_id,
  d.line_number,
  d.proposal_month AS io_proposal_month,
  d.raw_ingested_at,
